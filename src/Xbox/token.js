...
function requiresIdentityConfirmation(body) {
	try {
		...
		const m2 = (m1 || "").match(/identity\/confirm/).shift();

		return m2 !== null;
	} catch (err) {
		throw new Error(
			"Could not authenticate. Invalid credentials, incorrect email or password",
		);
	}
}
...
function loadSite(callback) {
	var post_vals = unescape(
		querystring.stringify(Object.assign({}, config.default.queries.authorize)),
	);

	axios
		.get(config.default.uris.authorize + "?" + post_vals, {
...
function getPreAuth(email, pass, callback) {
	var access_token_callback = function (response, urlPoster) {
		...
		let hash = "";
		const { responseUrl = "" } =
			((_a = response.request) === null || _a === void 0 ? void 0 : _a.res) ||
			{};
		...
		if (hash === void 0) {
			const errorMessage =
				requiresIdentityConfirmation(response.data) === true
		...
		callback({
			...
			expireDate: expires,
			scope: scope,
			nowCookie: response.headers["set-cookie"],
		});
	};
	...
	loadSite(data => {
		...
		axios(opts)
			.then(resp => access_token_callback(resp, data.url_post))
			.catch(console.log);
	});
}
...
function checkToken(token, callback) {
	...
	axios(opts)
		...
		.catch(err => {
			try {
				if (err.response.statusCode === 401) return callback(false);
